...
print(
    vars(obj)
)  # Wywołanie funkcji vars - musi odnosić się do obiektu, a ta musi mieć __init__()
...
from dataclasses import (
    dataclass,
)  # import metody potrzebnej do wykonania sposobu pierwszego
...
class Person:  # Tworzenie klasy i jej nazywanie (nawias może być pusty lub nawiązywać do jakiegpś obiektu bądź klasy)
    ...
    def speak(
        self,
    ):  # to jest metoda klasy- w zasadzie wszystkie ffunkcje w klasie poza  init i new są metodami    #wywoływanie jakiejś akcji z atrybutów  #nie trzeba podawać atrybutów
        print(
            f"Hi I am called {self.first_name} {self.last_name}, and my age is {self.age}"
        )
...
class Cat(Person):  # Dziedziczenie po klasie Person
    def __init__(
        self, first_name, age, color
    ):  # inicjalizowanie atrybutów po klasie z której ta klasa dziedziczy i dodawanie nowych atrybutów
...
class Point:
    ...
    def __add__(
        self, p
    ):  # w tej metodzie jeśli poza metodą mamy p1 + p2 to p2 staje się self a p1 p
        return Point(
            self.x + p.x, self.y + p.y
        )  # bierzemy koordynaty p1 i dodajemy je do koordynatów p2
        # odejmowanie
    ...
    def __str__(
        self,
    ) -> str:  # tametoda sprawia, że zamiast miejsca w pamięci printuje się obiekt
        return (
            f"({str(self.x)}, {str(self.y)})"  # nie dostajemy wówczas czegoś takiego:
        )
...
class Dog:
    ...
    def __init__(self, name):
        ...
        self.dogs.append(
            self
        )  # referencje do takiej zmiennej możesz robić tak samo jak do atrybutów
    ...
    def __str__(
        ...
        str
    ):  # nie musisz mieć referencji do czego kolwiek w klasie możemy mieć jakie parametry chcemy nie potrzeba w nich self ani cls
...
print(
    Dog.num_dogs()
)  # wywołanie classmethod - działa praktycznie tak samo jak wszystkie .strip() .join() .count()
Dog.bark(
    5
)  # wywołanie metody statycznej - używasz jej kiedy chesz użyc funcji jako funkcji chcesz mieć ją zapisaną w klasie
...
class KolorPodklasa(Kolor):
    @classmethod
    def szary(cls):
        return cls(120, 120, 120)
